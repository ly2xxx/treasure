...
def main():
    ...
    if 'hovered_id_column' not in st.session_state:
        ...
        if selected_data is not None:
            ...
            if st.session_state.hovered_id_column:
                ...
                if not matching_rows.empty:
                    # Update the session state for the selected treasure
                    st.session_state.selected_treasure = clicked_location
                    
                    # Update the selectbox value to match the clicked point
                    st.session_state.treasure_selector = clicked_location
                    
                    # Update map center and zoom to focus on the selected point
                    treasure_data = matching_rows.iloc[0]
                    ...
                    st.rerun()
            # Fallback to the original method if no hover data is available
            elif id_column in selected_data:
